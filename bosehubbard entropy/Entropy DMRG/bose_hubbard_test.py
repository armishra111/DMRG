...
def single_dmrg_step(model, sys, env, m, direction, diagonalize_superblock, target_sector=None, psi0_guess=None, callback=None):
    ...
    if target_sector is not None:
        ...
        #print(restricted_superblock_hamiltonian.A)
    ...
    #transformation_matrix = np.zeros((sys_enl.basis_size, my_m), dtype=model.dtype)
...
class DMRG(object):
    def __init__(self, model, L, m_warmup, diagonalize_superblock=arpack_diagonalize_superblock, target_sector=None, callback=None, graphic_callback=None):
        ...
        while 2 * block.length < L:
            ...
            if target_sector is not None:
                ...
                current_target_sector = target_sector
            else:
    ...
    def perform_measurements(self, measurements):
        ...
        print("taking expectation value...")
        returned_measurements = []
        for i, pm in enumerate(processed_measurements):
...
if __name__ == "__main__":
    ...
    def run_sample_bose_hubbard_chain(boundary_condition, L=32):
        now = datetime.now()
        L = 16
        N = 10
        d = N+1
        model = BoseHubbardChain(d=d, t=20, U=1, mu=0, boundary_condition=boundary_condition)
        measurements = ([[(i, "n")] for i in range(L)] +
                        [[(i, "n"), (j, "n")] for i in range(L) for j in range(L)])
        #measurements = ([[(i, "n")] for i in range(L)])               
        returned_measurements =  perform_dmrg(model, L=L, m_warmup=10, m_sweep_list=[10, 20, 30, 40], target_sector=N, measurements=measurements)
        
        dt_string = now.strftime("%d-%m-%Y %H %M %S")
        filename = "L={} N={} {}.csv".format(L,N, dt_string)
        np.savetxt(filename, returned_measurements, delimiter=',')
