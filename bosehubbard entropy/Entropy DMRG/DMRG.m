...
%--------------------------------------------------------------------------        
        ...
        function [sys_block, gnd_state, energy, rbi, T_matrix] = ...
            ...
            if isnan(conserved_QNum)
                rbi = 1:(sys_block.basis_size*env_block.basis_size);          
                for i=1: sys_QNum_Count
                    sys_QNum = sys_possible_QNum_List(i);
                    sys_QNum_Indices = sys_QNum_Indices_Map(sys_QNum);

                    new_sys_QNum_Indices_Map(sys_QNum) = [];
                    for j=1: length(sys_QNum_Indices)
                        k = sys_QNum_Indices(j);
                        currentIndices = new_sys_QNum_Indices_Map(sys_QNum);
                        start = sys_block.basis_size*(k-1)+1;
                        ends = env_block.basis_size*(k);
                        addedIndices = start:ends;
                        new_sys_QNum_Indices_Map(sys_QNum) = [currentIndices addedIndices];
                    end
                end
                restricted_superBlock_H = superBlock_H;
            
            elseif conserved_QNum > 0                
                rbi = [];  
                rbi_index = 1;
                for i=1: sys_QNum_Count
                    sys_QNum = sys_possible_QNum_List(i);
        ...
        %------------------------------------------------------------------       
            ...
            for i=1:new_sys_QNum_Indices_Map.Count
                if ~isempty(new_sys_Indices{i})
                    ...
                    self.reduce_densityM = blkdiag(self.reduce_densityM,rho_block);
